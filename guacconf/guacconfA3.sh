...
## Version A3
#
# Used for ROAM course at first. A new idea is examined - IPTABLES the openvpn to allow direct WINRM NAT (port 59433) to USER-DESKTOP.
# Additionally, a new Guacamole sequance is created for ROAM ESM "root:cmtsAdmin12#".
...
sudo /sbin/iptables -t nat -A PREROUTING -i eth0 -p tcp -j DNAT --to --dport 55986 10.200.1.10:5986
sudo /sbin/iptables -A FORWARD -p tcp --dport 5986 10.200.1.10 d j ACCEPT
